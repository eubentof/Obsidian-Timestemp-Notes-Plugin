...
export default class TimestempPlugin extends Plugin {
	...
	async insertTimestamp(): Promise<void> {
		...
		if (activeFile.basename === currentDate) {
			...
			const dailyNotePath: string = `Journal/Daily/${currentDate}`;
			const linkText: string = `[[${dailyNotePath}#^${timestamp}|${currentDate} ${currentTime}]]`;
	...
	async insertLineInDailyNote(
		...
		const dailyNotePath: string = `Journal/Daily/${currentDate}.md`;
		...
		if (dailyNote instanceof TFile) {
		  ...
		  const headerReference: string = `${currentDate}${timestamp.toString().slice(0, -3)}`;
		  const lineToAdd: string = `![[${originatingFileName}#⏲ * Journal/Daily/${currentDate} ${headerReference} ${currentDate.replace(/-/g, ' ')} *]]\n`;
	  
		  if (!fileContents.includes(lineToAdd)) {
			await this.app.vault.modify(dailyNote, fileContents + '\n' + lineToAdd);
		  }
		} else {
